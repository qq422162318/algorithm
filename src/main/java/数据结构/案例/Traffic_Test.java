...
import 数据结构.树.UF_TreeW;

import java.io.BufferedReader;
import java.io.InputStreamReader;

public class Traffic_Test {
    public static void main(String[] args) throws Exception{


        //构建一个缓冲读取流BufferedReader
        BufferedReader br = new BufferedReader(new InputStreamReader(Traffic_Test.class.getClassLoader().getResourceAsStream("数据结构/案例/traffic_project.txt")));

        //读取第一行数据20
        int totalNumber = Integer.parseInt(br.readLine());

        //构建一个并查集对象
        UF_TreeW uf = new UF_TreeW(totalNumber);
        //读取第二行数据7
